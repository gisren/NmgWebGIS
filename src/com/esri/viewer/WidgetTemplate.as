...
package com.esri.viewer
{
	...
	[Event(name="moveUpdate", type="flash.events.Event")]
	...
	public class WidgetTemplate extends SkinnableContainer implements IWidgetTemplate
	{
		...
		private static const WIDGET_MOVE_UPDATE:String = "moveUpdate";
		...
		{
			//2012.07.31 lxl +  派发窗体移动事件
			dispatchEvent(new Event(WIDGET_MOVE_UPDATE));
		...
		private function mouse_upHandler(event:MouseEvent):void
		{
			...
			var appObj:Object=FlexGlobals.topLevelApplication;
			...
			//add by wangl 2014 0707
			if(widget.x > (appObj.width - 150))
			{
				widget.x = appObj.width -150;				
			}
			// Add by xxl 2012.06.03 修改widget在图例显示的情况停靠不靠边的bug.
			var screenWidth:int = Capabilities.screenResolutionX;
			
			if(systemManager.stage.width-screenWidth<150)
			{
				if (widget.x >= (systemManager.stage.width - widget.width - 200 + 110))
				{
					widget.x = systemManager.stage.width - widget.width - 200 + 110;
				}
			}
			else
			{
		...
		public function removeAllTitleBar():void
		{
			headerToolGroup.removeAllElements();
		}
