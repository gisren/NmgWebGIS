	...
	public class ConfigManager extends EventDispatcher
	{
		...
		private function configService_resultHandler(event:ResultEvent):void
		{
			try
			{
				...
				//201206.20 lxl + 将几何服务的位置放到了webService标签下
				var geometryService:XML = new XML();
				for each(var geoService:XML in configXML.webService.service)
				{
					if(geoService.@name.toString() == "geometryService")
					{
				...
				var isWebServiceDyUrl:Boolean = configXML.webService.@dyUrl.toString() == "true" ? true : false;
				
//				var geometryService:XML = configXML.webService.geometryservice[0];
				if (geometryService)
				{
					var geoUrl:String = geometryService.@url[0] ? String(geometryService.@url[0]) : "";
					if(isWebServiceDyUrl){
						geoUrl = getDyUrl(geoUrl);
					}
					configData.geometryService.url = geoUrl; 
				...
				for (i = 0; i < controlList.length(); i++)
				{
					...
					var controlwIcon :String =controlList[i].@wIcon;
				...
				var isMapDyUrl:Boolean = configXML.map.@dyUrl.toString() == "true" ? true : false;
					...
					function webMapUtil_createMapByIdResultHandler(result:WebMapEvent):void
					{
						...
						for (i = 0; i < layers.length; i++)
						{
							...
							if (lyrXML)
							{
								if (isOpLayer)
								{
									configData.opLayers.push(getLayerObject(lyrXML, i, true, bingKey, webmapLayer, isMapDyUrl));
								}
								else
								{
									...
									configData.basemaps.push(getLayerObject(lyrXML, i, false, bingKey, webmapLayer, isMapDyUrl));
								}
							}
						}
						if (!initialExtent)
				...
				else
				{
					...
					{
						configMap.push(getLayerObject(maplayerList[i], i, false, bingKey, null, isMapDyUrl));
					}
					...
					{
						configOpLayers.push(getLayerObject(opLayerList[i], i, true, bingKey, null, isMapDyUrl));
					}
				...
				//webService
				//================================================
				var configService:Object = new Object();
				for each(var service:XML in configXML.webService.service)
				{
					var serverUrl:String = service.@url;
					
					if(isWebServiceDyUrl){
						serverUrl = getDyUrl(serverUrl);
					}
					
					var serviceName:String = service.@name;
					var serviceUrl:String = serverUrl;
					
					configService[serviceName] = serviceUrl;
				}
				configData.webService = configService;
				...
				function createWidgets(widgetList:XMLList, grouped:Boolean, groupLength:Number = 0, groupLabel:String = null, groupIcon:String = null):void
				{
					for (var p:int = 0; p < widgetList.length(); p++)
					{
						...
						var wLocation:String = widgetList[p].@position;
						var wVisible:String = widgetList[p].@visible;
				...
				for (i = 0; i < wContainers.length(); i++)
				{
					...
					function getWidgetList(wgList:XMLList, grouped:Boolean, groupLength:Number = 0, groupLabel:String = null, groupIcon:String = null):void
					{
						for (j = 0; j < wgList.length(); j++)
						{
							...
							var wgLeftIcon:String = wgList[j].@wIcon;
							...
							var sgVisible:String =  wgList[j].@visible;
		...
		private static function getLayerObject(obj:XML, num:Number, isOpLayer:Boolean, bingkey:String, webmapLayer:Layer = null, isDyUrl:Boolean = false):Object
		{
			...
			var name:String = obj.@name[0] ? obj.@name : obj.@label;
			...
			if(isDyUrl)
			{
				...
				//queryParams="http://192.168.5.66/ydzf/jl_ydzf_gis.html";
				//queryParams="http://192.168.5.66:2123/ydzf/jl_ydzf_gis.html";
				//queryParams="http://211.99.132.161/ydzf/jl_ydzf_gis.html";
				//queryParams="http://211.99.132.161:2123/ydzf/jl_ydzf_gis.html";
...
//				var outMapIPStr:String=webURL;
//				var outMapIPArr:Array=outMapIPStr.split('/');
//				var outIPStr:String=outMapIPArr[2].toString();
...
//				//把从浏览器中获取到的域名地址去掉端口 ；  by--wangl
//				var myparamsArray:Array=new Array();
//				if(paramsArray)
//				{
//					for(var x:int=0;x<paramsArray.length;x++)
//					{
//						if(x==2)
//						{
//							var ipString:String=paramsArray[x].toString();
//							var isPort:Boolean;
//							if( -1==ipString.indexOf(":",0) )
//							{
//								myparamsArray.push(ipString);
//							}
//							else
//							{
//								myparamsArray.push(ipString.substr(0,ipString.indexOf(":",0)));
//							}
//														
//						}
//						else
//						{
//							myparamsArray.push(paramsArray[x]);
//						}
//						
//					}
//				}
...
//				var dyUrl:String = "http://" + myparamsArray[2];
//				for(var m:int=3;m<urlArray.length;m++)
//				{
//					dyUrl = dyUrl+"/" + urlArray[m];
//				}
//				url = dyUrl;
		...
		/** 
		 * 
		 * 根据浏览器访问的域名，将配置的域名动态替换<br/>
		 * 前提：发布程序的服务器必须和替换的配置项在同一台机器上<br/>
		 * lxl 20120803 
		 * 
		 */ 
		public function getDyUrl(url:String):String
		{
			var queryParams:String = ExternalInterface.call("window.location.href.toString",1);
			var paramsArray:Array = queryParams.split('/');
			var urlArray:Array = url.split('/');
			var dyUrl:String = "http://" + paramsArray[2];
			for(var n:int=3;n<urlArray.length;n++)
			{
				dyUrl=dyUrl+"/" + urlArray[n];
			}	
		...
		/**
		 * 如果从浏览器中获取到的url地址带有端口，则去掉端口，然后改变地图服务的地址
		 * */
		private static function changeMapURL(mapurlStr:String):String
		{
			
			var IEurlStr:String=ExternalInterface.call("window.location.href.toString",1);
